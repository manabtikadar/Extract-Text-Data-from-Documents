...
model_ner = spacy.load(r'C:\Users\manab\OneDrive\Desktop\Project\App\output\model-best')
...
def get_predictions(image):
    ...
    if 'tokens' in doc_json and doc_json['tokens']:
        df_tokens = pd.DataFrame(doc_json['tokens'])
        df_tokens['tokens'] = df_tokens[['start', 'end']].apply(lambda x: doc_text[x[0]:x[1]], axis=1)
    else:
        print("No tokens found in the document.")
        return image, {}

    # Extract entities if present
    entity_table = pd.DataFrame(doc_json.get('ents', []))  # Avoid KeyError using `get()`
    if not entity_table.empty:
        entity_table = entity_table[['start', 'label']]
    else:
        return image, {}
